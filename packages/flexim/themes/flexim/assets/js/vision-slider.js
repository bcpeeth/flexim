...
$(document).ready(function($){
	//store DOM elements
	if (document.querySelector('.vision') != null) {
		...
		blockNavigation.on('click', 'a', function(e){
			e.preventDefault();
			e.stopPropagation();
			...
			if( !direction.hasClass('inactive') && !animating) {
		...
		function updateBlockNavigation(n) {
			$('.block-navigation__item--type-dot').removeClass('active');
			$('.block-navigation__item--type-dot').eq(n).addClass('active');
		...
		let isFrameHidden = true;
		const toggleButton = document.getElementById("displayFrame");

		let isFrameShowing = true;
		
		toggleButton.addEventListener('click', toggle);
		

		function hideFrame() {
			const elements = document.querySelectorAll(".toggleFrame");
			...
			toggleButton.innerHTML = "Show Possibility";
		...
		function toggle() {
			...
			if(isFrameHidden) {
				toggleButton.innerHTML = "Hide Possibility";
			}
			else {
				toggleButton.innerHTML = "Show Possibility";
			}
		...
		const dots = document.querySelectorAll(".block-navigation__item--type-dot");

		dots.forEach(function(element) {
			element.addEventListener('click', function(event) {
				const slideTo = element.getAttribute('data-slide-to');
				updateBlock(parseInt(slideTo, 10), MQ());
			});
		});
	}
});
