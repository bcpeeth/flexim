...
if ($c->isEditMode()){ ?>
  <div class="ccm-edit-mode-disabled-item"><?=t('Navigatie blok wordt niet getoond tijdens wijzigen.')?></div>
<?php }
elseif (count($navItems) > 0) {
    echo '<ul class="navbar text--align-center">'; //opens the top-level menu

    foreach ($navItems as $ni) {
        echo '<li class="' . $ni->classes . '">'; //opens a nav item
        echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="' . $ni->classes . '"><span class="navbar__item--text-default">' . h($ni->name) . '</span><span class="navbar__item--text-hover">' . h($ni->name) . '</span></a>';

        if ($ni->hasSubmenu) {
            echo '<ul displa>'; //opens a dropdown sub-menu
        } else {
